import './CheckPage.css'
import Button from '../../components/componentsSub/Button/Button'
import Model from '../../components/Model/Model';
import { right } from '../../assets/icon';


import { useContext } from 'react';
import { AppContext } from '../../Context/AppContext'
import useData from '../../hook/useData/useData';
import useLoad from '../../hook/useLoad/useLoad';

function CheckPage() {

    const { setActiveModel } = useContext(AppContext)
    const handleAddressModel = () => {
        setActiveModel('active')
    }
    const listProduct = useData('products')
    const product = useLoad(listProduct, 'checkPage')
    // console.log(product)
    return <div className="check-page">
        <div className="check-address">
            <div className="check-address__left">
                <div className="check-title">Địa chỉ nhận hàng</div>
                <div className="check-address__info">
                    <div className="check-address__info-name">
                        Tên: <span>Trung đức</span>
                    </div>
                    <div className="check-address__info-phone">
                        Số điện thoại: <span>038274385</span>
                    </div>
                    <div className="check-address__info-addr">
                        Địa chỉ: <span>hdcbghyssfbcjnifjifddok</span>
                    </div>
                </div>
            </div>
            <div className="check-address__right">
                <div className="btnAddress-mobile">{right}</div>
                <div onClick={handleAddressModel} className="btn__fix">Thay đổi</div>
                <Model>
                    <div className="toggleAddressModel">
                        <div className="addressModel-heading">Địa chỉ của tôi</div>
                        <div className="addressModel-list">
                            <div className="addressModel-info">
                                <div className="addressModel-info__check">
                                    <div className="select-address"></div>
                                </div>
                                <div className="addressModel-info__detail">
                                    <div className="addressModel-info__name">Trung đức</div>
                                    <div className="addressModel-info__phone">123433624638</div>
                                    <div className="addressModel-info__address">hdsajhnfidsjisaasijsckx</div>
                                </div>
                                <div className="addressModel-info__update">Cập nhật</div>
                            </div>
                        </div>
                        <div className="addressModel-add">Thêm địa chỉ mới</div>
                        <div className="addressModel-btns">
                            <div className="addressModel-btn close">Hủy</div>
                            <div className="addressModel-btn save">Xác nhận</div>
                        </div>
                    </div>
                </Model>
            </div>
        </div>

        <div className="check-product">
            <div className="check-product__head">
                <div className="check-title check-product__head-title">Sản phẩm</div>
                <div className="check-product__head-info">
                    <div className="check-product__head-iem">Đơn giá</div>
                    <div className="check-product__head-iem">Số lượng</div>
                    <div className="check-product__head-iem">Thành tiền</div>
                </div>
            </div>
            <div className="check-product__bott">
                <div className="check-product__bott-left">
                    <div className="check-product__bott-left-img">
                        <img src="data:image/jpeg;base64," alt="" />
                    </div>
                    <div className="check-product__bott-left-content">{product.name}</div>
                    <div className="check-product__bott-left-type">
                        Loại: 
                        <span>POLO 18,XXL</span>
                    </div>
                </div>
                <div className="check-product__bott-right">
                    <div className="check-product__bott-right-price">{product.price}</div>
                    <div className="check-product__bott-right-qual">1</div>
                    <div className="check-product__bott-right-sumPrice">118.000</div>
                </div>
            </div>
            <div className="check-product__note">
                <div className="check-product__note-letf">
                    <div className="check-product__note-letf-title">Lời nhắn: </div>
                    <input className='check-product__note-letf-input' type="text" name="" id="" placeholder='Lưu ý chonguoiwf bán ...' />
                </div>
                <div className="check-product__note-right">
                    <div className="check-product__note-right-transport">
                        <span>Đớn vị vận chuyển:</span> 
                        <div className="check-product__note-right-transport-detail">
                            <span>Nhanh</span>
                            <span className="check-product__note-right-transport-time">Nhận hàng vào 19 Th12 - 24 Th12</span>
                        </div>
                    </div>
                    <div className="check-product__note-right-fix btn__fix">Thay đổi</div>
                </div>
            </div>
        </div>

        <div className="check-pay">
            <div className="check-pay__head">
                <div className="check-title check-pay__head-title">Phương thức thanh toán</div>
                <div className="check-pay__method">
                    <span>Thanh toán khi nhận hàng</span>
                    <div className="check-pay__method-fix btn__fix">Thay đổi</div>
                </div>
            </div>
            <div className="check-pay__bott">
                <div className="check-pay__bott-left"></div>
                <div className="check-pay__bott-right">
                    <div className="check-pay__bott-priceProduct check-pay__mobile">
                        <span className="check-pay__bott-priceProduct-title">Số lượng</span>
                        <span className="check-pay__bott-priceProduct-money">2</span>
                    </div>
                    <div className="check-pay__bott-priceProduct check-pay__mobile">
                        <span className="check-pay__bott-priceProduct-title">Phân loại</span>
                        <span className="check-pay__bott-priceProduct-money">XL, xanh</span>
                    </div>
                    <div className="check-pay__bott-priceProduct">
                        <span className="check-pay__bott-priceProduct-title">Tổng tiền hàng</span>
                        <span className="check-pay__bott-priceProduct-money">236.000</span>
                    </div>
                    <div className="check-pay__bott-priceTransport">
                        <span className="check-pay__bott-priceTransport-title">Phí vận chuyển</span>
                        <span className="check-pay__bott-priceTransport-money">23.000</span>
                    </div>
                    <div className="check-pay__bott-priceResult">
                        <span className="check-pay__bott-priceResult-title">Tổng thanh toán</span>
                        <span className="check-pay__bott-priceResult-money">259.000</span>
                    </div>
                </div>
            </div>
            <div className="check-pay__btn">
                <Button text={'Đặt hàng'}/>
            </div>
        </div>
    </div>
}

export default CheckPage;